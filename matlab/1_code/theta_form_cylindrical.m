function tt = theta_form_cylindrical(r, amplitudes)
    zeta = zeta_generator(amplitudes);
    

    f_prime = @(theta) cos(theta) * (1 + zeta(theta)) - sin(theta)^2 * sum(amplitudes .* (collectdnPl(cos(theta), lmax = order, n = 1))[1:end]);
